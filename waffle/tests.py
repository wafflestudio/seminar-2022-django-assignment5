...
class TheLastTestCase(TestCase):
    ...
    def get_b_queryset(self) -> QuerySet:
        return Comment.objects.select_related('created_by', 'post').prefetch_related('tags')  # TODO

    def get_c_queryset(self) -> QuerySet:
        return PostSerializer.prefetch_queryset(Post.objects.select_related('created_by').prefetch_related('tags'))

        # return Post.objects.select_related(
        #     'created_by'
        # ).prefetch_related(
        #     'tags',
        #     Prefetch('comment_set',queryset=Comment.objects.select_related('created_by').prefetch_related('tags'))
        # )
